...
class FilterTest extends TestCase
{
    ...
    {
        $query = $this->filter->where(Field::column('col_a'), $this->operator->param()->eq('some_value'))->build();

        $this->assertEquals("WHERE `col_a` =?", trim($query->queryString()));
        $this->assertEquals(['some_value'], $query->parameters());
    }

    public function testWhereWithAlias()
    {
        $query = $this->filter->where(Field::column('t.col_a'), $this->operator->param()->eq('some_value'))->build();

        $this->assertEquals("WHERE `t`.`col_a` =?", trim($query->queryString()));
        $this->assertEquals(['some_value'], $query->parameters());
    }
    ...
    public function testWhereAndNestedWhereAndNestedWhere()
    {
        ...
        $query = $this->filter->where(Field::column('col_a'), $this->operator->param()->eq('some_value_a'))->nestedWhere(function () use ($operator) {
            /** @var FilterInterface $this */
            $this->where(Field::column('col_b'), $operator->param()->eq('some_value_b'));

            /** @var FilterInterface $this */
            $this->where(Field::column('col_c'), $operator->param()->eq('some_value_c'));

            /** @var FilterInterface $this */
            $this->nestedWhere(function () use ($operator) {
                /** @var FilterInterface $this */
                $this->where(Field::column('col_d'), $operator->param()->eq('some_value_d'));
            });
        })->where(Field::column('col_e'), $this->operator->param()->eq('some_value_e'))->build();

        $this->assertEquals("WHERE `col_a` =? AND ( `col_b` =? AND `col_c` =? AND ( `col_d` =? ) ) AND `col_e` =?", trim($query->queryString()));
    ...
    public function testNestedWhereOrNestedWhere()
    {
        ...
        $query = $this->filter->nestedWhere(function () use ($operator) {
            /** @var FilterInterface $this */
            $this->where(Field::column('col_a'), $operator->param()->eq('some_value_a'));

            /** @var FilterInterface $this */
            $this->nestedWhere(function () use ($operator) {
                /** @var FilterInterface $this */
                $this->orwhere(Field::column('col_b'), $operator->param()->eq('some_value_b'));
            });
        })->build();

        $this->assertEquals("WHERE ( `col_a` =? OR ( `col_b` =? ) )", trim($query->queryString()));
        $this->assertEquals(['some_value_a', 'some_value_b'], $query->parameters());
    }

    public function testWhereAndWhere()
    {
        $query = $this->filter->where(Field::column('col_a'), $this->operator->param()->eq('some_value_a'))->where(Field::column('col_b'), $this->operator->param()->eq('some_value_b'))->build();

        $this->assertEquals("WHERE `col_a` =? AND `col_b` =?", trim($query->queryString()));
        $this->assertEquals(['some_value_a', 'some_value_b'], $query->parameters());
    }

    public function testWhereOrWhere()
    {
        $query = $this->filter->where(Field::column('col_a'), $this->operator->param()->eq('some_value_a'))->orWhere(Field::column('col_b'), $this->operator->param()->eq('some_value_b'))->build();
    ...
    public function testWhereIn()
    {
        ...
        $this->assertEquals("WHERE `col_a` IN ( ?,? )", trim($query->queryString()));
        $this->assertEquals(['some_value_a', 'some_value_b'], $query->parameters());
    }
    ...
    public function testWhereInOrIn()
    {
        ...
        $this->assertEquals("WHERE `col_a` IN ( ?,? ) OR `col_a` IN ( ?,? )", trim($query->queryString()));
    ...
    public function testWhereNotIn()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` IN ( ?,? )", trim($query->queryString()));
        $this->assertEquals(['some_value_a', 'some_value_b'], $query->parameters());
    }
    ...
    public function testWhereNotInAndIn()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` IN ( ?,? ) AND NOT `col_b` IN ( ?,? )", trim($query->queryString()));
        $this->assertEquals(['some_value_a', 'some_value_b', 'some_value_c', 'some_value_d'], $query->parameters());
    }
    ...
    public function testWhereNotInOrIn()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` IN ( ?,? ) OR NOT `col_a` IN ( ?,? )", trim($query->queryString()));
        $this->assertEquals(['some_value_a', 'some_value_b', 'some_value_c', 'some_value_d'], $query->parameters());
    }

    public function testWhereNot()
    {
        $query = $this->filter->whereNot(Field::column('col_a'), $this->operator->param()->eq('some_value_a'))->build();

        $this->assertEquals("WHERE NOT `col_a` =?", trim($query->queryString()));
        $this->assertEquals(['some_value_a'], $query->parameters());
    }

    public function testWhereNotAndNot()
    {
        $query = $this->filter->whereNot(Field::column('col_a'), $this->operator->param()->eq('some_value_a'))->whereNot(Field::column('col_b'), $this->operator->param()->eq('some_value_b'))->build();

        $this->assertEquals("WHERE NOT `col_a` =? AND NOT `col_b` =?", trim($query->queryString()));
        $this->assertEquals(['some_value_a', 'some_value_b'], $query->parameters());
    }
    ...
    public function testWhereBetween()
    {
        ...
        $this->assertEquals("WHERE `col_a` BETWEEN ? AND ?", trim($query->queryString()));
    ...
    public function testWhereBetweenOrBetween()
    {
        ...
        $this->assertEquals("WHERE `col_a` BETWEEN ? AND ? OR `col_b` BETWEEN ? AND ?", trim($query->queryString()));
    ...
    public function testWhereBetweenAndBetween()
    {
        ...
        $this->assertEquals("WHERE `col_a` BETWEEN ? AND ? AND `col_b` BETWEEN ? AND ?", trim($query->queryString()));
        $this->assertEquals([1, 2, 3, 4], $query->parameters());
    }
    ...
    public function testWhereNotBetween()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` BETWEEN ? AND ?", trim($query->queryString()));
        $this->assertEquals([1, 2], $query->parameters());
    }
    ...
    public function testWhereNotBetweenOrNotBetween()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` BETWEEN ? AND ? OR NOT `col_b` BETWEEN ? AND ?", trim($query->queryString()));
        $this->assertEquals([1, 2, 3, 4], $query->parameters());
    }
    ...
    public function testWhereNotBetweenAndNotBetween()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` BETWEEN ? AND ? AND NOT `col_b` BETWEEN ? AND ?", trim($query->queryString()));
        $this->assertEquals([1, 2, 3, 4], $query->parameters());
    }
    ...
    public function testWhereExists()
    {
        ...
        $this->assertEquals("WHERE EXISTS ( SELECT * FROM some_table_name )", trim($query->queryString()));
    ...
    public function testWhereNotExists()
    {
        ...
        $this->assertEquals("WHERE NOT EXISTS ( SELECT * FROM some_table_name )", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }
    ...
    public function testWhereExistsOrExists()
    {
        ...
        $this->assertEquals("WHERE EXISTS ( SELECT * FROM some_table_name ) OR EXISTS ( SELECT * FROM some_table_name )", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }
    ...
    public function testWhereNotExistsOrNotExists()
    {
        ...
        $this->assertEquals("WHERE NOT EXISTS ( SELECT * FROM some_table_name ) OR NOT EXISTS ( SELECT * FROM some_table_name )", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }
    ...
    public function testWhereExistsAndExists()
    {
        ...
        $this->assertEquals("WHERE EXISTS ( SELECT * FROM some_table_name ) AND EXISTS ( SELECT * FROM some_table_name )", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }
    ...
    public function testWhereNotExistsAndNotExists()
    {
        ...
        $this->assertEquals("WHERE NOT EXISTS ( SELECT * FROM some_table_name ) AND NOT EXISTS ( SELECT * FROM some_table_name )", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }
    ...
    public function testWhereLike()
    {
        ...
        $this->assertEquals("WHERE `col_a` LIKE ?", trim($query->queryString()));
        $this->assertEquals(['%some_value%'], $query->parameters());
    }
    ...
    public function testWhereLikeOrLike()
    {
        ...
        $this->assertEquals("WHERE `col_a` LIKE ? OR `col_b` LIKE ?", trim($query->queryString()));
    ...
    public function testWhereLikeAndLike()
    {
        ...
        $this->assertEquals("WHERE `col_a` LIKE ? AND `col_b` LIKE ?", trim($query->queryString()));
        $this->assertEquals(['%some_value%','%some_value%'], $query->parameters());
    }
    ...
    public function testWhereNotLike()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` LIKE ?", trim($query->queryString()));
        $this->assertEquals(['%some_value%'], $query->parameters());
    }
    ...
    public function testWhereNotLikeOrNotLike()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` LIKE ? OR NOT `col_b` LIKE ?", trim($query->queryString()));
        $this->assertEquals(['%some_value%','%some_value%'], $query->parameters());
    }
    ...
    public function testWhereNotLikeAndNotLike()
    {
        ...
        $this->assertEquals("WHERE NOT `col_a` LIKE ? AND NOT `col_b` LIKE ?", trim($query->queryString()));
        $this->assertEquals(['%some_value%','%some_value%'], $query->parameters());
    }

    public function testWhereColEqualsAll()
    {
        $all = $this->operator->sql()->all(new Sql('select * from some_table',[],false))->sql();
        $query = $this->filter->where(Field::column('col_a'), $this->operator->sql()->eq($all, false))->build();
        $this->assertEquals("WHERE `col_a` = ALL (select * from some_table)", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }

    public function testWhereColEqualsAny()
    {
        $any = $this->operator->sql()->any(new Sql('select * from some_table',[],false))->sql();
        $query = $this->filter->where(Field::column('col_a'), $this->operator->sql()->eq($any,false))->build();
        $this->assertEquals("WHERE `col_a` = ANY (select * from some_table)", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }

    public function testWhereColEqualsSome()
    {
        $some = $this->operator->sql()->some(new Sql('select * from some_table',[],false))->sql();
        $query = $this->filter->where(Field::column('col_a'), $this->operator->sql()->eq($some, false))->build();
        $this->assertEquals("WHERE `col_a` = SOME (select * from some_table)", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }

    public function testWhereColEqualsExists()
    {
        $exists = $this->operator->sql()->exists(new Sql('select * from some_table',[],false))->sql();
        $query = $this->filter->where(Field::column('col_a'), $this->operator->sql()->eq($exists, false))->build();
        $this->assertEquals("WHERE `col_a` = EXISTS (select * from some_table)", trim($query->queryString()));
        $this->assertEquals([], $query->parameters());
    }
}
